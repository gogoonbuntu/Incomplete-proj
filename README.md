...
홈페이지의 프로젝트 정렬은 단순한 점수순이 아닌, **3가지 요소를 가중 합산한 추천 알고리즘**으로 동작합니다.

```
추천 점수 = 기본 스코어(40%) + 최신성(30%) + 유저 선호도(30%)
```

| 요소 | 가중치 | 설명 |
|------|--------|------|
| **기본 스코어** | 40% | AI가 분석한 프로젝트 품질 점수 (0~10점) |
| **최신성 (Recency)** | 30% | 최근 업데이트된 프로젝트일수록 높은 점수. 로그 감쇠 함수 사용 (30일 이내 ≈ 만점, 365일+ ≈ 0점) |
| **유저 선호도** | 30% | 개별 사용자의 클릭 히스토리 기반 맞춤 추천 |

### 유저 선호도 수집 방식

유저 선호도는 **서버에 저장하지 않고, 브라우저의 `localStorage`에만 저장**됩니다. 로그인 없이도 개인화된 추천이 가능하며, 사용자의 데이터는 절대 외부로 전송되지 않습니다.

#### 데이터 수집 시점
- 사용자가 **프로젝트 카드를 클릭**하여 상세 페이지로 이동할 때마다 해당 프로젝트의 속성이 기록됩니다.

#### 수집하는 데이터 (localStorage key: `user_pref`)

```json
{
  "lang:TypeScript": 5,    // TypeScript 프로젝트를 5번 클릭
  "lang:Python": 3,        // Python 프로젝트를 3번 클릭
  "cat:Web Development": 4, // 웹 개발 카테고리를 4번 클릭
  "topic:react": 2,        // react 토픽을 2번 클릭
  "topic:ai": 1            // ai 토픽을 1번 클릭
}
```

| 추적 항목 | 접두사 | 최대 boost | 설명 |
|-----------|--------|-----------|------|
| **프로그래밍 언어** | `lang:` | 15점 | 가장 큰 가중치. 사용자가 자주 보는 언어의 프로젝트를 우선 표시 |
| **카테고리** | `cat:` | 5점/개 | AI가 분류한 프로젝트 카테고리 (Web, CLI, Library 등) |
| **토픽** | `topic:` | 3점/개 | GitHub 토픽 태그 (react, machine-learning 등) |

#### 선호도 점수 계산

```typescript
// 각 항목의 클릭 수를 전체 최대 클릭 수 대비 비율로 정규화
preferenceScore = (clickCount / maxClickCount) * weight

// 전체 선호도 점수는 30점 상한
preferenceScore = Math.min(totalPreferenceScore, 30)
```

#### 프라이버시

- ✅ **100% 클라이언트 사이드**: 모든 선호도 데이터는 브라우저 `localStorage`에만 저장
- ✅ **외부 전송 없음**: 서버, 데이터베이스, 제3자에게 전혀 전송하지 않음
- ✅ **사용자 삭제 가능**: 브라우저 설정에서 사이트 데이터 삭제 시 즉시 초기화
- ✅ **익명**: 어떤 개인 식별 정보도 수집하지 않음
